from app.gateway.context import RequestContext
from app.gateway.detectors.base import PromptDetector
from app.gateway.detectors.normalization import normalize_prompt
from app.gateway.detectors.patterns import PROMPT_INJECTION_PATTERNS
from app.gateway.detectors.result import DetectorResult, DetectorStatus


class PromptInjectionDetector(PromptDetector):
    def analyze(self, context: RequestContext) -> DetectorResult:
        prompt = normalize_prompt(context.prompt)
        matches = [pattern for pattern in PROMPT_INJECTION_PATTERNS if pattern in prompt]
        if matches:
            return DetectorResult(
                detector_name=self.__class__.__name__,
                status=DetectorStatus.DANGEROUS,
                score=80,
                confidence=90,
                reason="Matched instruction override pattern.",
                matched_patterns=matches,
            )
        return DetectorResult(self.__class__.__name__, DetectorStatus.SAFE, 0, 100, "No instruction override pattern matched.")
